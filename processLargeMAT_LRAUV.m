function [ outfname ] = processLargeMAT_LRAUV( vehicle,year,searchterm,outmatfolder,varargin )
...
for k = 1:length(list)
    ...
    if ~any(~cellfun('isempty',regexp({d(~isub).name}, listf{fileIndex})))
        ...
        if ~yes2all
            ...
            if fsize>=1000
                str='';
                prompt = [listf{fileIndex} ' file size is ' num2str(fsize,4) 'MB. Proceed? YES/NO [YES]: '];
...
%         if isfield(vars,'CBIT')
%             vars.EFC = vars.CBIT.empericalClassifierFaultDetected;
%         end
        ...
        time=t1:tRes/(3600*24):t2; % new 0.4 sec grid
        
        % cut time vector for reshape bin avging (~5 sec bins)
        int = round(5/tRes); rsInd=0;
        while mod(length(time),int)~=0 && rsInd<200
            rsInd=rsInd+1;
            time(end) = [];
        end;
